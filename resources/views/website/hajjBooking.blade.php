			...
			<div class="db-2-main-com db-2-main-com-table">
				...
				@foreach($hajjBooking as $key)
						<tr>
							...
							<td><span style="font-size:12px;">{{$key['status']}}</span></td>
							<td> 
							        @if($key['status'] == "pending" && $key['payment_type'] == "Visa")
							            <a target="_blank" class="btn btn-primary btn-sm" href="/getPaymentPackage/{{$key['id']}}/hajj/web"> Complete Payment</a>
							            @else 
							               <p class="alert alert-info"> Contact With  Mangement </p>   
							            @endif
							</td>
							
							<th>$ {{$key['price']}}</th>
							<td><a href="/hajjBooking/{{$key['id']}}" class="db-done">view more</a>
							</td>
						</tr>
						@endforeach
					</tbody>
