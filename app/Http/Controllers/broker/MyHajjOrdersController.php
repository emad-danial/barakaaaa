...
class MyHajjOrdersController extends Controller
{
    /**
    ...
    public function index()
    {
        ...
        $records=DB::table('orders_package')
            ...
            ->where('umrahs.package_type','=','hajj')->where('orders_package.user_id','=',$userID)
...
//        $records = PackagesOrders::orderBy('id', 'desc')->paginate(20);
        return view('broker.my_hajj_orders.index', compact('records','pagename'));
    }
...
//        return redirect(route('rates.index'));
    ...
    public function show($id)
    {
       ...
       if($model->User->type == 'broker'){
        ...
        return view('broker.my_hajj_orders.show', compact('model','price','persons','package','companyName','Financialoperations','id')); 
         }else{
    ...
    public function edit($id)
    {
        ...
        $model = PackagesOrders::findOrFail($id);
        if($model->user_id == $userID ){
              return view('broker.my_hajj_orders.edit', compact('model'));
        }else{
     ...
     public function add($id)
    {
        ...
        if($model->user_id == $userID ){
              return view('broker.my_hajj_orders.add', compact('model'));
        }else{
